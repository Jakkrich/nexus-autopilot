...
entry point: Codex, Google Antigravity, Cursor, GitHub Copilot, Gemini CLI,
Aider, Zed, Windsurf, and others read `AGENTS.md`. Claude Code reads `CLAUDE.md`,
which imports this file, so there is a single source of truth.

## What this is

**AG Auto Click & Scroll** is a VS Code / Antigravity extension that automatically clicks action buttons (such as Run, Allow, Accept) and handles auto-scrolling in the Antigravity chat window with multi-instance support and real-time statistics.

This project is built with **Nexus Blueprint**, a workflow layer, not an
app skeleton.
...
- Codex and Google Antigravity: `.agents/skills/<skill>/SKILL.md`
- Claude Code: `.claude/skills/<skill>/SKILL.md`

Unused adapter families can be removed. Codex-only and Antigravity-only projects
keep `.agents/` and `AGENTS.md`, and can delete `CLAUDE.md` and `.claude/`.
...
adapter folders so Codex, Antigravity, and Claude Code stay aligned.
...
Code and Antigravity, use the slash commands (`/onboard`, `/overview`,
`/feature`, and so on). In tools without native skills, follow the matching
`SKILL.md` manually. The conventions in `blueprint/context/` apply however a
step is invoked.
...
## Commands
 ...
 - Tests: None configured yet (run `/tests` to configure unit testing)
 - Verification: None configured yet (run `/ci` to configure automated checks)
